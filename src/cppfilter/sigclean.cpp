...
const std::map<uint32_t, std::array<double, 4>> z_filters{
    {8000, {-1.414213562370318, 0.999999999996073, -1.386976835594689, 0.961481451595328}},
    {22050, {-1.919350454669626, 0.999999999998575, -1.905773886590930, 0.985852955569397}},
    {44100, {-1.979734945559178, 0.999999999999288, -1.972708333069861, 0.992901461374129}},
    {48000, {-1.982889722746972, 0.999999999999346, -1.976421874200365, 0.993476340642592}},
    {96000, {-1.995717846476881, 0.999999999999673, -1.992457692292373, 0.996732850597504}},
    {192000, {-1.998929174952568, 0.999999999999836, -1.997295141321751, 0.998365091018276}}};
...
void fft_filter_channel(std::vector<double> &audiodata, uint32_t sample_rate) {
    ...
    for (std::size_t i = 0; i + FFT_RES / 4 < audiodata.size(); i += FFT_RES / 4) {
        ...
        for (int j = -8; j < 8; j++) {
            in_transformed[(1000.0 * FFT_RES / sample_rate) + j][0] = 0;
            in_transformed[(1000.0 * FFT_RES / sample_rate) + j][1] = 0;
        }

        /* Revert to signal and overlap-add */
        auto backagain = backwards_fft(in_transformed);
        std::transform(backagain.begin(), backagain.end(), out.begin() + i, out.begin() + i, std::plus<double>());
    }

    /* Normalize signal */
    auto max = std::abs(*std::max_element(out.begin(), out.end(), [](double a, double b) { return (std::abs(a) < std::abs(b)); }));
...
void z_filter_channel(std::vector<double> &audiodata, const std::array<double, 4> &p) {
    ...
    for (int n = 2; n < audiodata.size(); n++) {
        filtered[n] = audiodata[n] + p[0] * audiodata[n - 1] + p[1] * audiodata[n - 2] - p[2] * filtered[n - 1] - p[3] * filtered[n - 2];
    }

    auto max = std::abs(*std::max_element(filtered.begin(), filtered.end(), [](double a, double b) {
        return (std::abs(a) < std::abs(b));
    }));
    std::transform(filtered.begin(), filtered.end(), filtered.begin(), [max](double d) { return d / max; });
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "[FFT] Processing " << input.getNumSamplesPerChannel() << " samples on " << input.getNumChannels() << " channels...\n";
    ...
    std::cout << "[Z] Processing " << input.getNumSamplesPerChannel() << " samples on " << input.getNumChannels() << " channels...\n";
